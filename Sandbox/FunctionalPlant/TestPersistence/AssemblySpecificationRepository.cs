...
public class AssemblySpecificationRepository : IReadOnlyRepository<AssemblySpecification>
{
    private IEnumerable<AssemblySpecification> Specs { get; }
    private Random Random { get; } = new(42);
    private PartsReadRepository PartsRepo { get; } = new();
    private Part[] Parts { get; }
    private const int SpecsCount = 15;

    public AssemblySpecificationRepository()
    {
        this.Parts = this.PartsRepo.GetAll().ToArray();
        this.Specs = Enumerable.Range(1, SpecsCount).Select(NextSpecification).ToArray();
    }
    ...
    public Option<AssemblySpecification> Find(Guid id)
        => Specs.FirstOrDefault(s => s.Id == id).Optional();
